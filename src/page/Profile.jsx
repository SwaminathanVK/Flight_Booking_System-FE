...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        console.log("User Data Response:", response); // <--- Add this to debug

        const user = response.data.user || response.data.data?.user; // Safe fallback
        const userTickets = response.data.tickets || response.data.data?.tickets;
        ...
        if(user){
          ...
          toast.error("Invalid user data receives.")
        }
